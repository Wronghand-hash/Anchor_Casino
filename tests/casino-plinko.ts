...
describe('casino_plinko', () => {
  ...
  it('Initializes player account and places a bet', async () => {
    ...
    await program.methods.initialize_player(new anchor.BN(100))
      .accounts({
        player_account: playerAccount.publicKey,
    ...
    let account = await program.account.player_account.fetch(playerAccount.publicKey) as PlayerAccount;
    expect(account.balance.toString()).to.equal('100');

    // Place a bet
    const gameAccount = anchor.web3.Keypair.generate();
    await program.methods.place_bet(new anchor.BN(50))
      .accounts({
        player_account: playerAccount.publicKey,
        game_account: gameAccount.publicKey,
        player: provider.wallet.publicKey,
        system_program: anchor.web3.SystemProgram.programId,
      })
      .signers([gameAccount])
    ...
    let game = await program.account.game_account.fetch(gameAccount.publicKey) as GameAccount;
    expect(game.betAmount.toString()).to.equal('50');
    expect(game.result).to.equal(0);

    // Determine the result
    await program.methods.determine_result(1)
      .accounts({
        game_account: gameAccount.publicKey,
        player_account: playerAccount.publicKey,
        player: provider.wallet.publicKey,
    ...
    account = await program.account.player_account.fetch(playerAccount.publicKey) as PlayerAccount;
